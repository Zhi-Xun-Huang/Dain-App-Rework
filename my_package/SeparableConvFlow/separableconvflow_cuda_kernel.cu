...
SeparableConvFlowLayer_gpu_forward_kernelfunc<<<grid,block,0, stream >>>(
			...
			input1.data_ptr<scalar_t>(),input2.data_ptr<scalar_t>(),input3.data_ptr<scalar_t>(), flow_output.data_ptr<scalar_t>()
			);
 					}));
...
//    cudaMemset((void*)gradinput3, 0, input3_b_stride * batch * sizeof(float));
	...
	SeparableConvFlowLayer_gpu_backward_kernelfunc <<<grid,block,0, stream>>>(
			...
			input1.data_ptr<scalar_t>(), 			input2.data_ptr<scalar_t>(),         input3.data_ptr<scalar_t>(),  			gradflow_output.data_ptr<scalar_t>(),
