...
//    printf("I am here\n");
	...
	AT_DISPATCH_FLOATING_TYPES_AND_HALF(input1.scalar_type(), "FlowProjection_gpu_forward_kernelfunc", ([&] {
	FlowProjection_gpu_forward_kernelfunc<<<grid,block,0, stream >>>(
			...
			input1.data_ptr<scalar_t>(),count.data_ptr<scalar_t>(),output.data_ptr<scalar_t>()
			);
								}));
...
//    printf("I am there\n");
	AT_DISPATCH_FLOATING_TYPES_AND_HALF(input1.scalar_type(), "FlowProjectionAveraging_kernelfunc", ([&] {
...
//        printf("use flow fill hole\n");
    	AT_DISPATCH_FLOATING_TYPES_AND_HALF(input1.scalar_type(), "FlowFillhole_kernelfunc", ([&] {
		...
		)
{
    ...
    if(BLOCKDIMX != 32 || BLOCKDIMY != 16||DEBUG)
        printf("BLOCKDIMX revised to %d, BLOCKDIMY revised to %d \n", BLOCKDIMX,BLOCKDIMY);
    	AT_DISPATCH_FLOATING_TYPES_AND_HALF(input1.scalar_type(), "FlowProjection_gpu_backward_kernelfunc", ([&] {
